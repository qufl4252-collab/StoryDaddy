...
export default function WriterPage() {
  ...
  async function generate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStory(result.story); setPage(0);
  ...
  return <main className="room writer-room">
    ...
    {!story ? <section className="writer-form-wrap"><p className="eyebrow">매일 새로운 한 권</p><h1>오늘은 어떤 세계로<br />떠나볼까요?</h1>
      ...
      </form></section> : <section className="storybook"><header><p>{story.theme}</p><h1>{story.title}</h1></header><article className="paper-page"><span>{page + 1}</span><p>{story.pages[page]}</p><small>{page + 1} / {story.pages.length}</small></article><div className="page-controls"><button onClick={() => setPage((n) => Math.max(0, n - 1))} disabled={page === 0}>← 이전 장</button>{page === story.pages.length - 1 ? <button onClick={() => setStory(null)}>새 동화 만들기</button> : <button onClick={() => setPage((n) => Math.min(story.pages.length - 1, n + 1))}>다음 장 →</button>}</div></section>}
  </main>;
}
